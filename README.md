# ⚡ Fiches de Révision – Électricité (Sketchnotes)

Une collection de fiches de révision sur les bases de l'électricité, réalisées au format **sketchnote** : un mélange de texte synthétique, de schémas et de dessins pour faciliter la mémorisation et la compréhension visuelle.

## 🎯 Objectif

Ces fiches rassemblent les notions fondamentales d'électricité utiles pour réviser rapidement avant un examen, une habilitation, ou tout simplement pour consolider ses bases. Chaque fiche traite d'un concept en une seule image, pensée pour être lue et comprise en quelques minutes.

## 🗂️ Contenu

### Grandeurs électriques de base
| Fiche | Sujet |
|---|---|
| [Tension](./Tension.png) | La tension électrique (U) |
| [Intensité](./Intensit%C3%A9.png) | L'intensité du courant (I) |
| [Résistance](./R%C3%A9sistance.png) | La résistance électrique (R) |
| [Puissance](./Puissance.png) | La puissance électrique (P) |
| [Quantité](./Quantit%C3%A9.png) | La quantité d'électricité |
| [AC - DC](./AC%20-%20DC.png) | Courant alternatif vs courant continu |

### Protection et sécurité
| Fiche | Sujet |
|---|---|
| [Disjoncteurs](./Disjoncteurs.png) | Rôle et fonctionnement des disjoncteurs |
| [Différentiels](./Diff%C3%A9rentiels.png) | Les interrupteurs différentiels |
| [Fusibles](./fusibles.png) | Les fusibles et leur rôle de protection |
| [Court circuit](./Court%20circuit.png) | Qu'est-ce qu'un court-circuit |
| [Prise de terre](./Prise%20de%20terre.png) | La mise à la terre et son importance |

### Matériel et vocabulaire
| Fiche | Sujet |
|---|---|
| [Conducteur](./conducteur.png) | Les conducteurs électriques |
| [Générateurs](./g%C3%A9n%C3%A9rateurs.png) | Les sources de courant |
| [Abréviation](./abr%C3%A9viation.png) | Les abréviations et symboles courants |

> 📁 Les liens ci-dessus supposent que les fichiers se trouvent à la racine du dépôt. Si tu les ranges dans un sous-dossier (par exemple `fiches/` ou `assets/`), adapte les chemins en conséquence.

## 🖊️ Pourquoi le format sketchnote ?

Le sketchnote combine texte, hiérarchie visuelle et petits dessins pour :
- Rendre l'information plus facile à mémoriser qu'un texte classique
- Permettre une lecture rapide, d'un coup d'œil
- Créer des repères visuels qui aident à se souvenir du contenu même après un premier passage

## 📖 Comment utiliser ces fiches

1. Parcours les fiches par thème selon ce que tu révises
2. Imprime-les ou garde-les sur un second écran pendant tes révisions
3. Reviens régulièrement dessus pour ancrer les notions dans la durée

## 🤝 Contribuer

Des suggestions de fiches à ajouter ou des corrections sont les bienvenues — ouvre une issue ou une pull request.

## 📄 Licence

*(à préciser selon ton choix — MIT, CC BY-NC, etc.)*
